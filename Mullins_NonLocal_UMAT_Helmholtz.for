...
!     statev(4) = nu_local ------ Local soft volume fraction
!     --------------------------------------------------------------
...
!**********************************************************************
     ...
     + props(nprops),coords(3),drot(3,3),dfgrd0(3,3),dfgrd1(3,3)
      ...
      real*8 theta_tau,theta_t,detF_tau,numax,nu_t_local,nu_tau_local
     ...
     +     F_tau,dtime,timeflag,theta_t,nu_tau_local,nu_t_local,
...
****************************************************************************
     ...
     +     nu_t,nu_tau,Achainmax,rpl,drpldt)
      ...
      real*8 theta_t,numax,X_tau,X_t,nu_tau_local,nu_t_local
      !
      ! Parameters
      !
      real*8 zero,one,two,half,three,third
     ...
     +     third=1.d0/3.d0)
      ...
      !
      ! Calculate the new ``local'' soft volume fraction
      !
      ! nu_tau = solvenu(nu_t,Amp,nu_ss,lamL,LambdaBar_tau,LambdaBar_t,Achainmax)
      !
      args(1) = nu_t_local
      args(2) = Amp
      args(3) = nu_ss
      args(4) = lamL
      args(5) = LambdaBar_tau
      args(6) = LambdaBar_t
      args(7) = Achainmax
      args(8) = dtime
      !
      if (flag.eq.one) then
      call solvenu(nu_tau_local,args,nargs)
      !
      ! ``Non-local'' soft volume fraction
      !
      nu_tau = theta_tau
      !
...
***********************************************************************
     ...
     +           Achainmax,varphi_t,jac)
      ...
      real*8 theta_t,numax,nu_tau_local,nu_t_local
      !
      ! Parameters
      !
      ...
      !
         ...
         if(tmp.le.zero) then
            ...
            enddo
               ...
               !
               call integ(Props,nprops,flag,numax,
     +        F_per,dtime,timeflag,theta_t,nu_tau_local,nu_t_local,
...
****************************************************************************
      ! This subroutine will numerically solve for the ``local'' soft volume fraction
      ! using rt safe from Numerical Recipes
      subroutine solvenu(root,args,nargs)
      !
      implicit none
      !
      ...
      real*8 LambdaBar_tau,LambdaBar_t,nu_t
      !
      parameter(maxit=1000)
!      parameter(xacc=1.d-9,zero=0.d0,one=1.d0)
      parameter(zero=0.d0,one=1.d0)
      !
      ! add some constraint on the tolerant
      !
      nu_t          = args(1)
      LambdaBar_tau = args(5)
      LambdaBar_t   = args(6)
      !
!      if(LambdaBar_tau - LambdaBar_t.EQ.zero) then
      xacc =  0.0001d0
!      else
!      xacc =  0.001*abs(LambdaBar_tau - LambdaBar_t)
!      endif
      !
      rootMax = 1.1d0
      rootMin = 0.1d0
      ...
      if(fl*fh.ge.zero) then
         write(*,*) 'FYI, root not bracketed on nu'
!         write(*,*) 'fl=',fl
!         write(*,*) 'x1=',x1
!         write(*,*) 'fh=',fh
!         write(*,*) 'x2=',x2
...
!         Write(*,*) 'nuss=',args(3)
!         Write(*,*) 'lamL=',args(4)
!         Write(*,*) 'lambda_new=',args(5)
!         Write(*,*) 'lambda_old=',args(6)
!         Write(*,*) 'Achainmax=',args(7)
!         call xit
         return
      endif
...
C
 ...
 10   CONTINUE
      ...
      WRITE(*,'(/1X,A)') 'solveRate EXCEEDING MAXIMUM ITERATIONS on nu'
      WRITE(80,'(/1X,A)') 'solveRate EXCEEDING MAXIMUM ITERATIONS on nu'
!      Write(*,*) 'F=', F
!      Write(*,*) 'Fl=', FL
!      Write(*,*) 'Fh=', FH
!      Write(*,*) 'Xl=', XL
...
!      call xit
      ...
      return
      end subroutine solvenu

****************************************************************************
      ! This is a utility subroutine for numerically solving for the 
      ! ``local'' soft volume fraction using Newton/Raphson
      subroutine gfunc(nu,f,df,args,nargs)

      implicit none

      integer nargs

      real*8 args(nargs),nu,f,df,nu_t,Amp
      real*8 nu_ss,lamL,LambdaBar_tau,LambdaBar_t,Achainmax
      real*8 X_tau,X_t,Achain_tau,Achain_t,aux1,aux2
      real*8 rate,Q,Rgas,theta,mustar,dtime
      !
      ...
      !
      X_tau = one + 3.5d0 * (one - nu) + 18.0d0 * (one - nu)**two
      X_t = one + 3.5d0 * (one - nu_t) + 18.0d0 * (one - nu_t)**two
      Achain_tau = dsqrt(X_tau*(LambdaBar_tau * LambdaBar_tau - one)
     +     +one)
      Achain_t = dsqrt(X_t*(LambdaBar_t * LambdaBar_t - one) + one)
      !
      f = Amp*(nu_ss - nu)*(lamL-one)*(Achain_tau-Achain_t)/
     +     (lamL-Achainmax)**two + nu_t - nu
      !
      ! Compute the tangent
      !
      df = (Amp*(lamL - one)*(Achain_t - Achain_tau))/
     +     (Achainmax - lamL)**two - one
      !
      return
